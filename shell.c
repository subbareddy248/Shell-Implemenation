...
#define dp1(x)       dup2(x,STDIN_FILENO)
#define dpo(x)    dup2(x, STDOUT_FILENO)
#define dppn(x)   dup2(x, 1)
#define dpppo(x)  dup2(x, 0)
#define cl(x)     close(x)
#define condition(a,b) if(execvp(a,b)<0) 
#define condition1(a)  if(a<0)
#define snd(ch)   scanf("%c",&ch);
#define conditionredirect(a,b) if(a==1 && b!=1)
#define loopcondition(a,b) while(a==' '|| b=='\t')
#define conditionflag(a) if(a==1)
#define tokenloop(a) while(a!=NULL)
#define conditionif(a) if(a!=NULL)
...
void prompt()
{	
	char *cwd=NULL,finalpath[1000];
	strc(finalpath,"~");
	cwd = getcwd(cwd,1000);
	char *path=strstr(cwd,MYHOME);
	path=path+strl(MYHOME);
	stra(finalpath,path);
	char *username,hostname[1024];
...
{
	int counter=0;
	tokenloop(arr[counter])
	{
		if(strm(arr[counter],"&"))	
		{
			counter++;
		}
		else if(!strm(arr[counter],"&"))	
		{
			arr[counter]=NULL;
			flag=1;
			break;
	...
	else if(!strncmp(arr[0],"history",7) && arr[0][7]!='\0')
	{
		char *start=(char*)malloc(sizeof(strl(start)+1));
		start=arr[0]+7;
		printf("%s",start);
		int len=strl(start);
		start[len]='\0';
	...
	{
		char *pPath;
		if(arr[0][1]=='P' && arr[0][2]=='A' && arr[0][3]=='T' && arr[0][4]=='H')
		{
			pPath = getenv("PATH");
		}
		else if(arr[0][1]=='H' && arr[0][2]=='O' && arr[0][3]=='M' && arr[0][4]=='E')
		{
			pPath=getenv("HOME");
		}
		if (pPath!=NULL)
	...
	else if((!strcmp(arr[0],"echo") && arr[1][0]=='$'))
	{
		char *pPath;
		if(arr[1][1]=='P' && arr[1][2]=='A' && arr[1][3]=='T' && arr[1][4]=='H')
		{
			pPath = getenv("PATH");
		}
		else if(arr[1][1]=='H' && arr[1][2]=='O' && arr[1][3]=='M' && arr[1][4]=='E')
		{
			pPath=getenv("HOME");
		}
		if (pPath!=NULL)
...
int main ()
{
	...
	{
		prompt();
		int i=0,pipeflag=0,redirectflag=0;
		...
		int spaces=0,temp=0;
		loopcondition(input[spaces],input[spaces])
		{
			spaces++;
		}
		while(input[temp]!='\0')
		...
		if((strm(hist[glob_count-1].name,input)!=0) && !sflag) 
		{
		...
		conditionredirect(redirectflag,pipeflag)
		{
			only_redirection(input);
			continue;
		}
		int k=strlen(input);
		char *input2=(char*)malloc(sizeof(k+1));
		strc(input2,input);
		if(strm(input2,"\0"))
		{
			parse(input2,arr);
...
{	
	...
	char *temp = NULL,*cmdArgs[1000],*newcmdArgs[1000];
	//newPipe[0]=read; newPipe[1]=write
	int newPipe[2], oldPipe[2], aCount=0, i, status,newaCount=0,in,out; 
	/* For each command */
	i=0;
	while(i<pipesCount) 
	{
		...
		if(pid == 0)  
		{
			...
			conditionflag(inflag)				
			{
				...
				else condition(cmdArgs[0],newcmdArgs) {
					erro("execvp");
					exit(1);
				}
			}
...
void only_redirection(char *input)
{
	...
	if(inCount >2 && outCount==2)				
	{
		...
		// child
			...
			else condition(cmdArgs[0],cmdArgs) {	
				erro("execl");
				exit(1);
			}		
		}	
